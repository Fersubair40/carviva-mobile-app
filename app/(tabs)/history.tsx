...
import TransactionCard from '@/components/ui/TransactionCard';
import Button from '@/components/ui/Button';
import { Filter, X, Calendar, Search } from 'lucide-react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import TransactionDetailsModal from '@/components/TransactionDetails';
import FilterModal from '@/components/FIlterContainer';
import { useGetHistory } from '@/api/queries/history';
...
export default function HistoryScreen() {
  ...
  const { data, isLoading, isRefetching, refetch } = useGetHistory({
    page: 1,
    limit: 20,
  });

  const [transactions, setTransactions] = useState<Reports[]>([]);
  const [selectedTransaction, setSelectedTransaction] =
  ...
  const handleTransactionPress = (transaction: Reports) => {
    setSelectedTransaction(transaction);
    setModalVisible(true);
  };
  ...
  const groupTransactionsByDate = () => {
    const grouped = transactions.reduce(
      (groups: Record<string, Reports[]>, transaction) => {
